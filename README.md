...
**当前是源码迁移版本，尚未公开分发安装包。** 本仓库不内嵌生产协议认证材料、用户会话或维护者签名身份。全新安装连接官方服务前需要在本机配置有权使用的协议认证材料；详见 [开发配置](docs/DEVELOPMENT.md)。仅编译和离线测试不需要这些材料。
...
```sh
./tools/bootstrap.sh       # 下载校验固定 SDK，解析锁定依赖，生成原创资源
...
这是社区项目，不是 OOPZ 官方 macOS 发行版。OOPZ 服务和相关标识属于其权利人。本项目源码及原创生成资源采用 [MIT](LICENSE) 许可；第三方 SDK 和服务不因此变为 MIT 授权，详见第三方说明。

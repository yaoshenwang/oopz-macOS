...
已有本机应用数据中的协议材料仍可使用。新用户通过独立的官方网页登录页登录：WebLoginBridge 观察该页面原有的 WebCrypto 协议密钥导入及登录会话，原样调用原始 WebCrypto 函数，不修改其参数或可导出属性。原生端只接收 HTTPS 官方主框架发来的消息，并检查密钥格式；不读取用户的浏览器个人资料。真实首次登录需要人工验收，官方页面改变实现时可能需要更新衔接。

维护者也可以从有权使用的来源取得 PKCS#1 / PKCS#8 DER 文件，通过下述无头命令导入，或通过 `OOPZ_PROTOCOL_KEY_FILE` 指定外部文件。

```sh
build/latest/Oopz.app/Contents/MacOS/Oopz --import-protocol-key "$OOPZ_PROTOCOL_KEY_FILE"
```

导入不会把密钥写进 App，它仅进入应用私有数据文件。然后由用户启动 GUI，通过官方网页登录完成自己的账号登录。没有已有材料时，应先完成官方网页登录；无头集成检查仍要求本机具备有效会话和协议材料。

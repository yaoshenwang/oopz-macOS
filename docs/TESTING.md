# 测试

## 分层

| 时机 | 命令 | 能证明什么 |
| --- | --- | --- |
| 编辑／PR | tools/check_fast.sh | PCM 数值、权限状态机、日志隐私、协议规范化、畸形 DER、官方登录脚本隔离、发布审计、验证记录约束和共享音频配置 |
| PR | tools/build.sh --assemble + ci_release.py prepare | 双架构构建、产物审计、发布输入校验，不涉及产品登录或签名身份 |
| 版本标签 | Release workflow | Developer ID 双架构签名、Apple 公证、真实 DMG 挂载内容与无头启动 |
| 发版准备 | tools/verify.sh --media | universal 本地签名构建、真实协议检查和生产共享发布流程 |
| 已有同一构建 | tools/verify.sh --media --existing | 不重建，先验证当前源文件和完整 App 文件清单是否匹配 |
| 共享／声音发布前 | 官方 Web 人工验收 | 对端实际画面和听感；不能用本地 PASS 替代 |

任何必要前置条件缺失都失败：没有登录、无自有域、频道有人、RTC 入会超时等。所有自动测试单实例、无窗口、不激活、不采集真实麦克风或桌面。合成音频只用于进程内 RMS，不进入 SDK。

本地验证会结束旧的原生实例，再直接启动二进制。账号只进入 `owner == uid` 的自有空语音频道。日志及详细结果保存在私有临时目录，`build/<版本>/validation.json` 保存验证收据；这些文件不上传 CI 或 GitHub。

## 官方 Web 人工验收

在本地自动检查结束后，由用户启动原生端账号 A。官方 Web 使用不同账号 B，加入 A 已验证的自有测试域。两端真实画面与声音由用户确认：

1. 全屏共享：B 可发现并看到持续更新的画面。
2. 停止：B 的画面和共享入口正确结束。
3. 窗口共享：B 看到所选窗口，不是其他屏幕内容。
4. 晚加入：共享开始后 B 再进入，仍能发现并观看。
5. 系统音频：真实声音清晰、可停止、无回声或循环播放。

v0.4.0 的第 5 项还需验证：共享电脑声音开启时，共享者人声只出现一次；闭麦只停人声；暂停共享声音只停应用声音；麦克风、共享发送、成员语音及共享收听音量分别生效。检查停止重开、晚加入和悬浮音频面板交互。详细步骤见 [音频说明](AUDIO.md)。

两端优先使用不同电脑和耳机，避免同机浏览器的通话回放被系统声音重新采集。配置检查不创建 RTC 引擎、不采音、不推送声音；不证明真实音频通过。

结果写入仓库外 JSON，包含与 validation.json 相同的 `sourceCommit`、`sha256`，以及 `fullscreen`、`stop`、`window`、`late_join`、`system_audio` 五个字段。实际通过才填写字符串 `pass`。不得由自动化臆造验收结果。

外部服务失败时记录为失败／未验收。不要为了绿色状态跳过测试，也不自动操作官方 Web 页面。

首次登录衔接：用户手动在官方登录页完成验证，确认原生主界面进入成功；自动检查仅验证脚本行为和来源约束，不代操作验证码。
